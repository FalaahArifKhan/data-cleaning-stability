...
def get_scatter_plot_data(missingness_types: list, dataset_to_column_name: dict,
    ...
    # Merge on colA, colB, colC and take only colE from df2
    merged_df = pd.merge(avg_imputation_quality_metrics_df, avg_model_performance_metrics_df[['Dataset_Name', 'Null_Imputer_Name', 'Evaluation_Scenario', 'Missingness_Type', model_performance_metric_name]],
                         on=['Dataset_Name', 'Null_Imputer_Name', 'Evaluation_Scenario', 'Missingness_Type'],
                         how='left')

    return merged_df, new_imputation_quality_metric
...
def create_scatter_plot(missingness_types: list, dataset_to_column_name: dict,
    ...
    scatter_plot = alt.Chart(merged_df).mark_circle(size=100).encode(
