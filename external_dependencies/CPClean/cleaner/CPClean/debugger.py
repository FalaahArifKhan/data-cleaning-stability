...
class Debugger(object):
    ...
    def init_log(self, percent_cc):
        self.gt_val_acc = KNNEvaluator(self.data["X_train_gt"], self.data["y_train"],
                                       self.data["X_val"], self.data["y_val"]).score()
        self.X_train_mean = deepcopy(self.data["X_train_repairs"]["mean_mode"]
                                     if "mean_mode" in self.data["X_train_repairs"] else self.data["X_train_repairs"]["mean"])
        self.selection = []

        self.logging = []
        self.dc_val_acc = KNNEvaluator(self.X_train_mean, self.data["y_train"],
                                       self.data["X_val"], self.data["y_val"]).score()
    ...
    def log(self, n_iter, sel, sel_time, percent_cc):
        ...
        cpclean_val_acc = KNNEvaluator(self.X_train_mean, self.data["y_train"],
                                       self.data["X_val"], self.data["y_val"]).score()
