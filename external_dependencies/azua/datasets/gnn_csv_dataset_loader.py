...
# TODO #20934: The code is currently mostly copy & pastes of GNNSparseCSVDatasetLoader and inherits from CSVDatasetLoader.
class GNNCSVDatasetLoader(CSVDatasetLoader):
    """
    ...
    def _get_graph_data(self, dataset: Dataset, model_config: Optional[Dict[Any, Any]]):
        """
        Create torch_gemetric.data.Data from Dataset.

        Args:
            dataset: Dataset containing train, test, val data with user, item entries.
    ...
    def _create_df_with_recalibrated_row_idxs(
        self,
        data_and_mask: Tuple[np.ndarray, np.ndarray],
        idxs: List[int],
    ) -> Optional[DataFrame]:
        """
        ...
        Args:
            ...
            idxs: train, test, val idxs that origianlly appear in Dataset, e.g., dataset.data_split["train_idxs"]
