...
T = TypeVar("T", np.ndarray, csr_matrix)
...
class DataProcessor:
    ...
    def process_dataset(self, dataset: Union[Dataset, SparseDataset]) -> Union[Dataset, SparseDataset]:
        ...
        if type(dataset) is SparseDataset or type(dataset) is Dataset:
            return type(dataset)(
    ...
    def process_data(self, data: np.ndarray) -> np.ndarray:
        ...
        if self._cts_unproc_cols:
            for unproc_region, proc_region, normalizer in zip(
                ...
                processed_data[:, proc_start:proc_end] = normalizer.fit_transform(cts_unproc_data)
